...
def main():
  ...
  # Read and validate the raw timeline data file(s).
  raw_timeline = []
  last_end_time = None
  for raw_name in args.raw:
    ...
    last_end_time = validate_raw(raw, raw_name, last_end_time)
    ...
    # into a new list.
    extract_and_append_timeline(raw, raw_timeline)

  # Read the annotated file and any cache files.
...
def parse_args():
  ...
  parser.add_argument('-r', '--raw', required=True, action='append',
      help='Raw Google timeline input JSON file (may be repeated).'
        'Specify older timeline input files before newer ones.')
...
def validate_raw(raw, raw_name, last_end_time):
  """
  Validate our assumptions about the format of the raw timeline data.  Raise
  an error and exit if any of the checks fail.

  raw           - Contents of the raw timeline data file.
  raw_name      - Name of the raw timeline data file.
  last_end_time - The datetime of the last "endTime" record of the previous raw
                   input file, or None if there was no previous file.

  Returns the datetime of the last "endTime" record in this raw input file.
  """
  ...
  for rec in raw:
    ...
    if 'timelinePath' in rec:
      ...
      if last_end_time and start < last_end_time:
...
def extract_and_append_timeline(raw, recs):
  """
  Extract the "timelinePath" records from the raw timeline data.
  Append these records to "recs".
  """
  for rec in raw:
    if 'timelinePath' in rec:
      r = {}
  ...
  for arec in annotated:
    ...
    for atlrec in atimeline:
      ...
      while irpoint2 < len(rtimeline) and \
            aioff <= int(rtimeline[irpoint2]['durationMinutesOffsetFromStartTime']) and \
